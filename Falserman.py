...
UPPER_ABCS = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
LOWER_ABCS = "abcdefghijklmnopqrstuvwxyz"
FULLY_ABCS = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
NUMBER = "1234567890"
...
class Spawner:
    ...
    def phonenumber(count:int, lenght:int=8) -> list:
        """
        :param count: Quantity produced
        :param lenght: The length of the resulting number
        """

        Result = []
        # The following replacement needs to be reduced by one bit
        lenght -= 1
        for i in range(count):
    ...
    def password(count:int, lenght:int=0, inString:bool=True, inNumber:bool=True, inSymbol:bool=True):
        ...
        if count == 0 or TC.isInt(lenght) == False:
            return []
        ...
        for i in range(count):
            for j in range(lenght):
                __tempPassword += random.choice(__tempWords)
            Result.append(__tempPassword)
        return Result
